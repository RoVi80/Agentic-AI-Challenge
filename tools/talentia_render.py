"""Talentia-branded PowerPoint renderer.

Used by the rolando_style_clone_agent in IBM watsonx Orchestrate. Also runnable
as a CLI for local testing. Reads the same JSON shape as siro_simple_render.js:
  { "title", "subtitle", "settings": {...}, "slides": [ ... ] }

Run locally:
    python tools/talentia_render.py prototype/sample_deck.json prototype/output.pptx
"""
...
DEFAULTS = {
    ...
    "logoBase64": "",             # if extracted from a reference deck, used in place of the embedded Talentia logo
}
...
def _add_logo(slide, x, y, width, logo_b64=""):
    """Drop a logo at (x, y); height auto-scales from the PNG.

    If logo_b64 is provided (extracted from the user's reference deck), use it.
    Otherwise fall back to the embedded Talentia primary logo so we never
    produce a logo-less title slide on a Talentia-default flow.
    """
    b64 = logo_b64 if logo_b64 else LOGO_PRIMARY_B64
    try:
        img_bytes = base64.b64decode(b64)
        img_stream = io.BytesIO(img_bytes)
        slide.shapes.add_picture(img_stream, Inches(x), Inches(y), width=Inches(width))
    except Exception:
        # If the extracted blob is malformed, fall back to embedded logo silently
...
def _title_slide(slide, spec, s):
    ...
    # No logo — extraction is unreliable across decks (most brands draw the wordmark
    # as text on the master, not as a PNG). Cleaner across any brand without one.
    # Big title
...
def render_to_bytes(deck):
    """Render a presentation dict to .pptx bytes."""
...
def _safe_filename(name):
    name = (name or "presentation").strip()
...
@tool(
    name="talentia_render",
    description=(
        "Renders a presentation JSON into a brand-compliant Talentia-styled PowerPoint "
        "and returns it as a downloadable .pptx file. "
        "Input shape: { 'title', 'subtitle', 'settings' (fontFace, titleColor, bodyColor, "
        "backgroundColor, accentColor, footerText — hex without #), 'slides' (array) }. "
        "Each slide has type 'title' | 'bullets' | 'two_column' | 'closing'."
    ),
)
def talentia_render(presentation: dict, output_filename: str = "presentation.pptx") -> bytes:
    # output_filename is accepted for forward compatibility but the bytes-return path
    # in Orchestrate auto-names the download — the parameter is currently unused.
    return render_to_bytes(presentation)
...
if __name__ == "__main__":
    ...
    file_bytes = render_to_bytes(deck)
    Path(args.output).parent.mkdir(parents=True, exist_ok=True)
    Path(args.output).write_bytes(file_bytes)
    print(f"Wrote {args.output} ({len(file_bytes):,} bytes)")
